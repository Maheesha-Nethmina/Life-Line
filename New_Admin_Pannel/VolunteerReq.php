    ...
    public function getVolunteers()
    {
        try {
            if (isset($_GET['search'])) {
                $search = $_GET['search'];
                $sql = "SELECT volunteer.volunteerId, volunteer.phone, volunteer.address, volunteer.imageOfResult, 
         ...
         AND user.userType = 'User'";
                ...
                $sql = "SELECT volunteer.volunteerId, volunteer.phone, volunteer.address, volunteer.imageOfResult, 
                user.userName, user.userEmail, user.created_At,volunteer.pre,volunteer.nic,volunteer.age,volunteer.district,volunteer.imageOfPloiceReport,user.userId 
         FROM volunteer 
         JOIN user ON volunteer.userId = user.userId
...
<body>
    ...
    <div class="sidebar">
        ...
        <a href="/New_Admin_Pannel/Admin_conrol.php"><i class="fa-solid fa-handshake-angle"></i> Check
            Volunteer</a>
    ...
    <div class="content container-fluid">
        <div class="table-container">
            ...
            <h3 class="text-center">Check Volunteer Requests</h3>
            <div class="d-flex justify-content-between mb-3">
                <a href="/New_Admin_Pannel/Admin_conrol.php"> <button class="btn btn-success"><i
                            class="fas fa-plus-circle"></i> Manage Volunteer</button></a>
            </div>
            <table class="table table-striped table-hover">
                <thead>
                    <tr>
                        ...
                        <th class="col-1">A/L result</th>
                        <th class="col-2">Address</th>
                        <th class="col-2">Police Report</th>
                        <th class="col-2">previous volunteering</th>
                    ...
                    if (!empty($volunteers)) {
                        foreach ($volunteers as $index => $row): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="upload/<?= htmlspecialchars($row['imageOfResult']) ?>" download="<?= htmlspecialchars($row['imageOfResult']) ?>">
                                        <img src="upload/<?= htmlspecialchars($row['imageOfResult']) ?>" class="img-fluid" style="width: 80px;">
                                    </a>
                                </td>
                                <td><?= htmlspecialchars($row['address']) ?></td>
                                <td>
                                    <a href="upload/<?= htmlspecialchars($row['imageOfPloiceReport']) ?>" download="<?= htmlspecialchars($row['imageOfPloiceReport']) ?>">
                                        <img src="upload/<?= htmlspecialchars($row['imageOfPloiceReport']) ?>" class="img-fluid" style="width: 80px;">
                                    </a>
                                </td>
                                <td><?= htmlspecialchars($row['pre']) ?></td>
                                <td><?= htmlspecialchars($row['phone']) ?></td>



                                <td>
                                    <a href='Delete.php?reqId=<?= htmlspecialchars($row['volunteerId']) ?>'
                                    ...
                                    <a href='Email.php?reqId=<?= htmlspecialchars($row['volunteerId']) ?>'
                                        class='btn btn-outline-secondary btn-sm' title="Send Email"
                                        style="margin-bottom: 5px;"><i class="fa-solid fa-envelope"></i></a>

                                    <a href='Accept_Vol_Request.php?id=<?= htmlspecialchars($row['volunteerId']) ?>' class='btn btn-outline-success btn-sm' title="Accept Volunteer Request"
                                        style="margin-bottom: 5px;" onclick="return confirm('Are you sure you want to accept this volunteer request?');"><i class="fa-solid fa-square-check"></i></a>
