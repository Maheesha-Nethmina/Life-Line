...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    if (empty($Name) || empty($NIC) || empty($Phone) || empty($Address) || empty($Image) || empty($District) || empty($Age)) {
        header("location:/New_Admin_Pannel/Edit_Beneficiary.php?id=$id&message=2");
    } else {
        $sql = "UPDATE benificiary SET patientName = ?, nicNo = ?, address = ?, district = ?, gardian_No = ?, age = ?, image = ? WHERE benificiaryId = ?";
        $stmt = $conn->prepare($sql);
        $stmt->execute([$Name, $NIC, $Address, $District, $Phone, $Age, $update_file, $id]);
